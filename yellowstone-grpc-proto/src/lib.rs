...
pub mod convert_to {
    ...
    pub fn create_message_from_nats(message: &NatsSanitizedMessage) -> proto::Message {
        match message {
            NatsSanitizedMessage::Legacy(NatsLegacyMessage { message, .. }) => proto::Message {
                header: Some(create_header_from_nats(&message.header)),
                account_keys: create_pubkeys_from_nats(&message.account_keys),
                recent_blockhash: message.recent_blockhash.to_bytes().into(),
                instructions: create_instructions_from_nats(&message.instructions),
                versioned: false,
                address_table_lookups: vec![],
            },
            NatsSanitizedMessage::V0(NatsLoadedMessage { message, .. }) => proto::Message {
                ...
                instructions: create_instructions_from_nats(&message.instructions),
                versioned: true,
                address_table_lookups: create_lookups_from_nats(&message.address_table_lookups),
            },
        }
    }
    ...
    pub fn create_pubkeys_from_nats(pubkeys: &[NatsPubkey]) -> Vec<Vec<u8>> {
        pubkeys
            .iter()
            .map(|key| <Pubkey as AsRef<[u8]>>::as_ref(&Pubkey::from(key)).into())
            .collect()
    }
    ...
    pub fn create_transaction_meta_nats(
        ...
        let err = create_transaction_error_from_nats(status);
        let inner_instructions_none = inner_instructions.is_none();
        let inner_instructions = inner_instructions
            .as_deref()
            .map(create_inner_instructions_vec_from_nats)
            .unwrap_or_default();
        let log_messages_none = log_messages.is_none();
        let log_messages = log_messages.clone().unwrap_or_default();
        let pre_token_balances = pre_token_balances
        ...
        proto::TransactionStatusMeta {
            ...
            loaded_writable_addresses,
            loaded_readonly_addresses,
            return_data: return_data.as_ref().map(create_return_data_from_nats),
            return_data_none: return_data.is_none(),
            compute_units_consumed: *compute_units_consumed,
        }
    }
    ...
    pub fn create_rewards_from_nats(rewards: &[NatsReward]) -> Vec<proto::Reward> {
        rewards.iter().map(create_reward_from_nats).collect()
    }
    ...
    pub fn create_reward_from_nats(reward: &NatsReward) -> proto::Reward {
        proto::Reward {
            pubkey: reward.pubkey.clone(),
            lamports: reward.lamports,
            post_balance: reward.post_balance,
            reward_type: create_reward_type_from_nats(reward.reward_type.clone()) as i32,
            commission: reward.commission.map(|c| c.to_string()).unwrap_or_default(),
        }
    }
    ...
    pub const fn create_reward_type_from_nats(
        ...
        match reward_type {
            ...
            Some(NatsRewardType::Rent) => proto::RewardType::Rent,
            Some(NatsRewardType::Staking) => proto::RewardType::Staking,
            Some(NatsRewardType::Voting) => proto::RewardType::Voting,
        }
    }
